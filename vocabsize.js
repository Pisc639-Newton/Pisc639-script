(async function() {
    ...
    for (var percentage = 1; percentage <= 100; percentage += 0.01) {try {var divs = $('.desktop-screen div').filter(function() {var style = $(this).attr('style');return style === "background: linear-gradient(to top, #679ABF, #6D6E71 " + percentage + "%);border-radius:15px;"|| style === "background: linear-gradient(to bottom, #6D6E71 100%, #679ABF " + percentage + "% );border-radius:15px;";});if (divs.length > 0) {divs[0].click();break;}} catch (error) {console.error(error);}}
